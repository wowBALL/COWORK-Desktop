...
//   '#' ที่ขึ้นต้นค่าตรง ๆ (ไม่มีช่องว่างนำหน้า) ทำให้ src/glossary.py ข้ามทั้งบรรทัดทิ้งเป็น
//       comment เต็มบรรทัด (เคส "#redmine-support" ในรีวิว) -- คนละเคสกับ " #" กลางคำด้านบน
//   \n หรือ \r (และตัวแบ่งบรรทัดอื่นที่ splitlines() ของ Python ตัดด้วย) แทรกบรรทัดใหม่เข้า
//       กลางไฟล์ตรง ๆ -- ช่องกรอกคำถูก/คำผิดเป็น <input> ปกติพิมพ์ผ่านคีย์บอร์ดไม่ได้ แต่วาง
//       จากคลิปบอร์ดหลายบรรทัดได้
...
// splitlines() ของ Python ตัดบรรทัดกว้างกว่าแค่ CR/LF -- \v \f \x1c \x1d \x1e \x85 (NEL)
// U+2028 (line separator) U+2029 (paragraph separator) ก็ทำให้ Python ตัดบรรทัดเหมือนกัน ค่าที่มี
// ตัวใดตัวหนึ่งปนมา (ปกติมาจากการวางคลิปบอร์ด ไม่ใช่พิมพ์ตรง ๆ) จะรอด /[\r\n]/ เดิมไปได้
// แต่ Python จะอ่านกลับมาเป็นค่าที่ถูกตัดครึ่งบวกบรรทัดแปลกปลอมอีกบรรทัด ทั้งที่ UI รายงานว่าสำเร็จ
const NEWLINE_RE = /[\r\n\v\f\x1c\x1d\x1e\x85\u2028\u2029]/;
function findUnsafeChar(s) {
  const str = String(s || '');
  if (str.includes(':')) return ':';
  if (str.includes(',')) return ',';
  if (NEWLINE_RE.test(str)) return '\\n';
  // '#' ที่ "ขึ้นต้น" ค่า (คนละเคสกับ " #" กลางคำด้านล่าง) -- str ที่เข้ามาถึงนี่ผ่าน .trim()
  // มาแล้วเสมอ (ดู planWrite: term/form ถูก .trim() ก่อนส่งเข้าฟังก์ชันนี้ทุกจุด) ดังนั้น "ขึ้นต้นด้วย #"
  // กับ "มีช่องว่างนำหน้า #" คนละเคสกันเป๊ะ ไม่ทับซ้อนกัน -- src/glossary.py ข้ามทุกบรรทัดที่ trim()
  // แล้วขึ้นต้นด้วย '#' ทิ้งเป็น comment เต็มบรรทัด (ไม่ใช่แค่ inline comment กลางบรรทัดแบบ " #")
  // C#/F# ไม่โดนเช็คนี้เพราะ '#' ของมันไม่ได้อยู่ตำแหน่งแรกของสตริง
